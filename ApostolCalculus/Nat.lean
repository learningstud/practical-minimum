...
protected instance decEq: (a b: ℕ) → Decidable (a = b)
  ...
  | succ a, succ b => @decidable_of_decidable_of_iff _ _ (a.decEq b) succ_eq_succ
...
protected instance decLe: (a b: ℕ) → Decidable (a ≤ b)
  ...
  | succ a, succ b => @decidable_of_decidable_of_iff _ _ (a.decLe b) succ_le_succ
...
protected theorem lt_irrefl: (a: ℕ) → ¬a < a
  | 0 => ℕ.not_lt_zero 0
  | succ a => fun h => a.lt_irrefl (ℕ.succ_le_succ.mpr h)
...
#check Nat.sub_zero
@[simp] theorem monus_zero (a: ℕ): a.monus 0 = a := rfl

#check Nat.zero_sub
@[simp] theorem zero_monus: (a: ℕ) → ℕ.monus 0 a = 0
  | 0 => rfl
  | succ a => congrArg ℕ.pred a.zero_monus

theorem monus_succ (a b: ℕ): a.monus b.succ = (a.monus b).pred := rfl

#check Nat.succ_sub_succ_eq_sub
#check Nat.succ_sub_succ
theorem succ_monus_succ (a: ℕ): (b: ℕ) → a.succ.monus b.succ = a.monus b
  | 0 => rfl
  | succ b => congrArg ℕ.pred (a.succ_monus_succ b)

#check Nat.sub_self
@[simp] theorem monus_self: (a: ℕ) → a.monus a = 0
  | 0 => rfl
  | succ a => (a.succ_monus_succ a).trans a.monus_self

#check Nat.sub_eq_zero_of_le
theorem monus_eq_zero_of_le {a b: ℕ}: a ≤ b → a.monus b = 0
  | .refl => a.monus_self
  | .step h => congrArg ℕ.pred (monus_eq_zero_of_le h)

#check Nat.sub_le
theorem monus_le (a: ℕ): (b: ℕ) → a.monus b ≤ a
  | 0 => .refl
  | succ b => trans (a.monus b).pred_le (a.monus_le b)

#check Nat.sub_lt
theorem monus_lt: {a b: ℕ} → 0 < a → 0 < b → a.monus b < a
  | 0, _, ha, _ => (ℕ.not_lt_zero 0 ha).elim
  | succ _, 0, _, hb => (ℕ.not_lt_zero 0 hb).elim
  | succ a, succ b, _, _ =>
    suffices a.monus b < a.succ from a.succ_monus_succ b ▸ this
    ℕ.succ_le_succ.mp (a.monus_le b)

#check Nat.sub_ne_zero_of_lt
theorem sub_ne_zero_of_lt {a b: ℕ}: a < b → b.monus a ≠ 0 := sorry

#check Nat.add_sub_of_le
theorem add_monus {a b: ℕ} (h: a ≤ b): a + b.monus a = b := by
  match a with
  | 0 => simp
...
theorem monus_add {a b: ℕ} (h: a ≤ b): b.monus a + a = b :=
  by rw [ℕ.add_comm, add_monus h]

/-
theorem le_of_monus_eq_zero: {a b: ℕ} → a.monus b = 0 → a ≤ b
  | 0, b, _ => b.zero_le
...
theorem div_algo {a b: ℕ}: 0 < b ∧ b ≤ a → a.monus b < a
  | ⟨hb, hba⟩ => monus_lt (trans hb hba) hb

#check ℕ.lt_wfRel
#check Nat.div
/-- Truncated division -/
protected def tdiv (a b: ℕ): ℕ :=
  if 0 < b ∧ b ≤ a then ((a.monus b).tdiv b).succ else 0
termination_by tdiv a b => a
decreasing_by apply div_algo; assumption
-- decreasing_by simp_wf; exact monus_lt (trans h.1 h.2) h.1

#check ℕ.rec
#check ℕ.recOn
#check Nat.div.inductionOn
protected def tdivRec
  {motive: ℕ → ℕ → Sort _}
  (base: (a b: ℕ) → ¬(0 < b ∧ b ≤ a) → motive a b)
  (ind: (a b: ℕ) → 0 < b ∧ b ≤ a → motive (a.monus b) b → motive a b)
  (a b: ℕ)
: motive a b
:=
  if h: 0 < b ∧ b ≤ a
  then ind a b h (ℕ.tdivRec base ind (a.monus b) b)
  else base a b h
termination_by tdivRec _ _ a _ => a
decreasing_by exact div_algo h

protected def tdivRecOn
  {motive: ℕ → ℕ → Sort _}
  (a b: ℕ)
  (base: (a b: ℕ) → ¬(0 < b ∧ b ≤ a) → motive a b)
  (ind: (a b: ℕ) → 0 < b ∧ b ≤ a → motive (a.monus b) b → motive a b)
: motive a b
:= ℕ.tdivRec base ind a b

#check Nat.modCore
#check Nat.mod
/-- Modulo where `0 (mod n)` is NOT judgmentally equal to `0`. -/
protected def mod (a b: ℕ): ℕ :=
  if 0 < b ∧ b ≤ a then (a.monus b).mod b else a
termination_by mod a b => a
decreasing_by apply div_algo; assumption
...
#check Nat.mod_eq
theorem mod_eq (a b: ℕ): a % b = if 0 < b ∧ b ≤ a then (a.monus b) % b else a :=
  by rw [←ℕ.mod.def, ℕ.mod]; rfl

protected theorem mod.pos {a b: ℕ} (h: 0 < b ∧ b ≤ a): a % b = a.monus b % b :=
  mod_eq a b ▸ if_pos h
...
theorem mod_eq_sub_mod {a b: ℕ} (h: a ≥ b): a % b = a.monus b % b :=
