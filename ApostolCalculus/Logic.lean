...
-/
def ExistsUnique {α} (p: α → Prop) := ∃ x, p x ∧ ∀ y, p y → y = x
...
def AtMostOne {α} (p: α → Prop) := ¬∃ x y: α, x ≠ y ∧ p x ∧ p y
...
theorem all_then_not_exists_not {α} {p: α → Prop} (h: ∀ x, p x): ¬∃ x, ¬p x
  | ⟨x, hnp⟩ => hnp (h x)
example {α} {p: α → Prop} (h: ¬∀ x, p x) : ∃ x : α, ¬p x := sorry
theorem not_exists_not {α β} {p: α → β → Prop}: (∀ a b, p a b) → ¬∃ a, ∃ b, ¬p a b
  | h, ⟨a, b, hnp⟩ => hnp (h a b)
...
example (p: Prop): [Decidable p] → p ∨ ¬p := if h: p then .inl h else .inr h
  -- | isTrue h => .inl h
  -- | isFalse h => .inr h
#check 1 - 1